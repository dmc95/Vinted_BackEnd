...
router.post("/user/signup", async (req, res) => {
  ...
  try {
    // recherche de l'adresse mail dans la bdd
  ...
  try {
    ...
    if (user) {
      ...
      if (
        ...
        SHA256(password + user.salt).toString(encBase64) ===
        user.hash
      ) {
        res.status(200).json({
          _id: user.id,
...
router.put("/user/update/:id", async (req, res)=>{
  ...
  try{
    ...
    if (req.fields.phone) {
      userToModify.account.phone = req.fields.phone;
    }else{
      return res.json({message:"phone not modify"})
    }
